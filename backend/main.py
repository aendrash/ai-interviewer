...
def retrieve_context(query: str, k=3):
    """Retrieve relevant context from FAISS vector store, supports all FAISS versions."""
    ...
    try:
        # Old versions: returns (D, I)
        distances, labels = index.search(query_emb, k)
    except TypeError:
        try:
            # New FAISS 1.12 API (returns labels separately)
            labels = index.search(query_emb, k, distances)
        except Exception as e:
            print("FAISS search failed:", e)
            return "Error during vector search. Please check FAISS index."

    if labels is None or len(labels[0]) == 0:
        return "No relevant documents found in FAISS index."

    retrieved_docs = [docs[i] for i in labels[0] if 0 <= i < len(docs)]
    return "\n\n---\n\n".join(retrieved_docs)
